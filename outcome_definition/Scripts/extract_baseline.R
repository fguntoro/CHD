...
if (length(mylist_codes) > 0) {
  ...
  for (i in 1:length(mylist_codes)) {
    tmp <- fread(paste0(def_path, "/", mylist_codes[i]), data.table = FALSE, tmpdir = "~/../ephemeral/")
    tmp <- tmp[, 1]
  ...
  print(app_data_path)
  column_names <- colnames(fread(app_data_path, data.table = FALSE, nrows = 0, tmpdir = "~/../ephemeral/"))
