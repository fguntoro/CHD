...
tmp <- fread(app_data_path, data.table = FALSE, select = which(column_names %in% c("eid", "53-0.0")), tmpdir = "~/../ephemeral/")
...
print(hes_main_path)
hes_full <- fread(hes_main_path, data.table = FALSE, tmpdir = "~/../ephemeral/")
print("Loading HES diagnosis dataset from:")
print(hes_diag_path)
hes <- fread(hes_diag_path, data.table = FALSE, tmpdir = "~/../ephemeral/")
print("Loading HES operations dataset from:")
print(hes_oper_path)
hes_oper <- fread(hes_oper_path, data.table = FALSE, tmpdir = "~/../ephemeral/")
