...
mydata <- readRDS(paste0(out_path, "output_hes.rds"))
...
death_full <- fread(death_main_path, data.table = FALSE, tmpdir = "~/../ephemeral/")
print("Loading cause of death dataset from:")
print(death_cause_path)
death <- fread(death_cause_path, data.table = FALSE, tmpdir = "~/../ephemeral/")
...
# Saving outputs
saveRDS(mydata, paste0(out_path, "output_registries.rds"))
