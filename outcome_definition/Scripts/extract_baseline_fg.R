...
def_path <- "/rds/general/user/fg520/home/Cancer/outcome_definition/Definitions/cancer1/"
app_data_path <- "/rds/general/project/hda-22-23/live/TDS/General/Data/ukb669759.csv"
out_path <- "/rds/general/user/fg520/home/Cancer/outcome_definition/Outputs/cancer1/"
...
if (length(mylist_codes) > 0) {
  ...
  # field_columns <- field_ids # Getting all instances
...
test_that("All incident cases have a date of/time to diagnosis", {
  expect_equal(sum(mydata$incident_case[is.na(as.numeric(mydata$date_diagnosis_baseline))], na.rm = TRUE), 0)
  expect_equal(sum(mydata$incident_case[is.infinite(as.numeric(mydata$date_diagnosis_baseline))], na.rm = TRUE), 0)
  expect_equal(sum(is.na(mydata$date_diagnosis_baseline[which(mydata$incident_case == 1)])), 0)
  expect_equal(sum(is.infinite(mydata$date_diagnosis_baseline[which(mydata$incident_case == 1)])), 0)
...
# Saving outputs
saveRDS(mydata, paste0(out_path, "output_final.rds"))
