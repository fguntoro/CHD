...
# def_path <- "/rds/general/user/fg520/home/Cancer/outcome_definition/Definitions/cancer4/"
# app_data_path <- "/rds/general/project/hda-22-23/live/TDS/General/Data/ukb669759.csv"
# out_path <- "/rds/general/user/fg520/home/Cancer/outcome_definition/Outputs/cancer4/"


# ifelse(dir.exists(out_path), FALSE, dir.create(out_path))
...
mydata <- data.frame(
  eid = names(date_recr),
  date_recr = date_recr,
  date_diagnosis_baseline = as.Date(rep(NA, length(date_recr))),
  date_diagnosis_latest = as.Date(rep(NA, length(date_recr))),
  ...
  if (length(full_list) > 0) {
    ...
    for (k in 1:length(full_list)) {
      ...
      if (length(tmp) > 0) {
        ...
        mydata[tmp_eid, "date_diagnosis_baseline"] <- as.Date(ifelse((mydata[tmp_eid, "date_diagnosis_baseline"] >= epistart) | (is.na(mydata[tmp_eid, "date_diagnosis_baseline"])),
                                                            yes = epistart,
                                                            no = mydata[tmp_eid, "date_diagnosis_baseline"]
        ),
        origin = "1970-01-01"
        )
        # Updating first date of diagnosis if before the current one
        mydata[tmp_eid, "date_diagnosis_latest"] <- as.Date(ifelse((mydata[tmp_eid, "date_diagnosis_latest"] <= epistart) | (is.na(mydata[tmp_eid, "date_diagnosis_latest"])),
                                                            yes = epistart,
                                                            no = mydata[tmp_eid, "date_diagnosis_latest"]
        ),
        origin = "1970-01-01"
        )
        mydata[tmp_eid, paste0(full_list[k])] <- as.Date(epistart,origin = "1970-01-01")
...
prevalent_ids <- which(mydata[, "date_diagnosis_baseline"] < mydata[, "date_recr"])
mydata[prevalent_ids, "prevalent_case"] <- 1
mydata[prevalent_ids, "incident_case"] <- NA
incident_ids <- which(mydata[, "date_diagnosis_baseline"] >= mydata[, "date_recr"])
...
mydata[incident_ids, "time_to_diagnosis"] <- mydata[incident_ids, "date_diagnosis_baseline"] - mydata[incident_ids, "date_recr"]

# Saving outputs
saveRDS(mydata, paste0(out_path, "output_hes.rds"))
if (length(unknown_eids) > 0) {
  saveRDS(unknown_eids, paste0(out_path, "unclear_prevalent_incident.rds"))
}
